...
describe("calculate smart move sequence", () => {
  ...
  describe("[free rules] backdoLaunch: on, yutMoCatch: on, shortcutOptions: on", () => {
    ...
    it("r0s2, r1sH, u0s5, u1s6, u2u3s20, gul, should sequence to r0s2 -> r0s5 (catch)", () => {
      let friendlyPieces = JSON.parse(JSON.stringify(initialState.initialPiecesTeam0))
      friendlyPieces[0] = {
        tile: 2,
        team: 0,
      ...
      expect(bestMoveSequence).toEqual([
        {
          tokenId: 0,
          moveInfo: { tile: 5, move: '3', history: [0, 1, 2, 3, 4], path: [2, 3, 4, 5] }
        },
      ])
    })
    it("r0s5, r1s6, r0r1s20, r2s9, r3s10, gul, should sequence to r1s6 -> r1s9 (catch)", () => {
      let friendlyPieces = JSON.parse(JSON.stringify(initialState.initialPiecesTeam0))
      friendlyPieces[0] = {
        tile: 5,
        team: 0,
      ...
      expect(bestMoveSequence).toEqual([
        {
          tokenId: 1,
          moveInfo: { tile: 9, move: '3', history: [0, 1, 2, 3, 4, 5, 6, 7, 8], path: [6, 7, 8, 9] }
        },
      ])
    })
  })
  describe("[Korean rules] backdoLaunch: off, yutMoCatch: off, shortcutOptions: off", () => {
    it("r0s2 r1sH, u0s4, yut+ge, should sequence to r0s2 -> r0s4 (catch), and r1sH -> r1s4", () => {
      let friendlyPieces = JSON.parse(JSON.stringify(initialState.initialPiecesTeam0))
      friendlyPieces[0] = {
        tile: 2,
        team: 0,
      ...
      expect(bestMoveSequence).toEqual([
        {
          tokenId: 0,
          moveInfo: { tile: 4, move: '2', history: [0, 1, 2, 3], path: [2, 3, 4] }
        },
        {
          tokenId: 1,
          moveInfo: { tile: 4, move: '4', history: [0, 1, 2, 3], path: [0, 1, 2, 3, 4] }
...
describe("calculate score", () => {
  describe("backdoLaunch: on, yutMoCatch: on, shortcutOptions: on", () => {
    describe("start", () => {
      it("should return 6 if you start and send a ship to star 1", () => {
        let friendlyPieces = JSON.parse(JSON.stringify(initialState.initialPiecesTeam0))
        friendlyPieces[0] = {
          tile: 1,
          team: 0,
        ...
        let score = calculateScore({ pieces: friendlyPieces, enemyPieces, backdoLaunch, throwsEarned: 0, numCaught: 0, shortcutOptions: true })
        expect(score).toEqual(6)
      })
      it("should return 6 if you start and send a ship to star 2", () => {
        let friendlyPieces = JSON.parse(JSON.stringify(initialState.initialPiecesTeam0))
        friendlyPieces[0] = {
          tile: 2,
          team: 0,
      ...
      it("should return 5 if you start and send a ship to star 3", () => {
        let friendlyPieces = JSON.parse(JSON.stringify(initialState.initialPiecesTeam0))
        friendlyPieces[0] = {
          tile: 3,
          team: 0,
      ...
      it("should return -21 if you use a backdo to send a ship to Earth", () => {
        let friendlyPieces = JSON.parse(JSON.stringify(initialState.initialPiecesTeam0))
        friendlyPieces[0] = {
          tile: 0,
          team: 0,
    ...
    describe("piggyback", () => {
      it("should return 13 if you have two ships on star 2", () => {
        let friendlyPieces = JSON.parse(JSON.stringify(initialState.initialPiecesTeam0))
        friendlyPieces[0] = {
          tile: 2,
          team: 0,
    ...
    describe("multiple friendlies", () => {
      // stack
      // separated
      it("should return 13 if you have a ship at s1 and another ship at s2", () => {
        let friendlyPieces = JSON.parse(JSON.stringify(initialState.initialPiecesTeam0))
        friendlyPieces[0] = {
          tile: 1,
          team: 0,
    ...
    describe("compare two scores", () => {
      ...
      it("should favor a token on s10 and token on s6 over a token on s9 and another one on s7", () => {
        ...
        let backdoLaunch = true
        let score0 = calculateScore({ pieces: friendlyPieces0, enemyPieces: enemyPieces0, backdoLaunch, throwsEarned: 0, numCaught: 0, shortcutOptions: true })
        ...
        backdoLaunch = true
        let score1 = calculateScore({ pieces: friendlyPieces1, enemyPieces: enemyPieces1, backdoLaunch, throwsEarned: 0, numCaught: 0, shortcutOptions: true })
      ...
      // 072725: in first row, favor advancing in first row over piggyback
      ...
      // 072725: favor advancing ship that's outside of first row over sending a new one
      it("should favor u0s23, r0s4, r1s4, r2s4 over u0s21, u1s2, r0s4, r1s4, r2s4", () => {
        let friendlyPieces0 = JSON.parse(JSON.stringify(initialState.initialPiecesTeam0))
        friendlyPieces0[0] = {
          tile: 21,
          team: 0,
  ...
  describe("backdoLaunch: off, yutMoCatch: off, shortcutOptions: off", () => {
  })
})